...
const OrderCard = (props) => {
   ...
   const updateOrder = () => {
    ...
    axios.put(`${baseURL}order/${props.id}`, order, config)
    .then((res) => {
...
<View style={[{backgroundColor: cardColor},styles.container]}>
    <View style={styles.container}>
        <Text>מספר הזמנה: {props.id}</Text>
        <Text>מספר הזמנה: {props.orderItems.map((x) => {
           return (
         
           <Text key={x._id}>{x.product.business}</Text>
           )
        })}</Text>
    ...
    <View style={{marginTop: 10}}>
        ...
        <Text> תאריך: {props.dateOrdered.split("T")[0]}  </Text>
